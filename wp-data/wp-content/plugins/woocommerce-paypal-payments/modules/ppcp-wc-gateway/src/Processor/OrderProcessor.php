...
class OrderProcessor {
	...
	public function process( WC_Order $wc_order ): void {
        ...
        if( $market && $market == 'uk' ) {
            $this->settings->set( 'merchant_email_sandbox', 'sales@globalparts.co.uk');
            $this->settings->set( 'merchant_email_production', 'sales@globalparts.co.uk');
            $this->settings->set( 'merchant_email', 'sales@globalparts.co.uk');

            $this->settings->set( 'merchant_id_sandbox', '2HUEUJTCBD4H2');
            $this->settings->set( 'merchant_id_production', '2HUEUJTCBD4H2');
            $this->settings->set( 'merchant_id', '2HUEUJTCBD4H2');

            $this->settings->set( 'client_id_sandbox', 'AZZjjgueRBKik67-VMWxevs1x4cIKKVSxq5nGU3jPNH0gZSsUy6XYmoXn9V-UmQP57ZH0S_6fSRzoDWU');
            $this->settings->set( 'client_id_production', 'AZZjjgueRBKik67-VMWxevs1x4cIKKVSxq5nGU3jPNH0gZSsUy6XYmoXn9V-UmQP57ZH0S_6fSRzoDWU');
            $this->settings->set( 'client_id', 'AZZjjgueRBKik67-VMWxevs1x4cIKKVSxq5nGU3jPNH0gZSsUy6XYmoXn9V-UmQP57ZH0S_6fSRzoDWU');

            $this->settings->set( 'client_secret_sandbox', 'EBZbR2EDqwUm1ICTX9QNpeB3uFFjs59R673fL1d-H4Y9dM4cdTAbYPbKlNZYXCNh4SgIGewHGzgpezN4');
            $this->settings->set( 'client_secret_production', 'EBZbR2EDqwUm1ICTX9QNpeB3uFFjs59R673fL1d-H4Y9dM4cdTAbYPbKlNZYXCNh4SgIGewHGzgpezN4');
            $this->settings->set( 'client_secret', 'EBZbR2EDqwUm1ICTX9QNpeB3uFFjs59R673fL1d-H4Y9dM4cdTAbYPbKlNZYXCNh4SgIGewHGzgpezN4');
        } else {
            $this->settings->set( 'merchant_email_sandbox', 'kontakt@globalparts.com.pl');
            $this->settings->set( 'merchant_email_production', 'kontakt@globalparts.com.pl');
            $this->settings->set( 'merchant_email', 'kontakt@globalparts.com.pl');

            $this->settings->set( 'merchant_id_sandbox', 'IDZ232MA6JZM2');
            $this->settings->set( 'merchant_id_production', 'IDZ232MA6JZM2');
            $this->settings->set( 'merchant_id', 'IDZ232MA6JZM2');

            $this->settings->set( 'client_id_sandbox', 'AXGm-wTjYLj_BHh8pSG8NwrkwJ99LA6K6M4a9LHQLNSE2BVK5NljgPlz0pYVIxLfS1V9Aq9qB70C9CHY');
            $this->settings->set( 'client_id_production', 'AXGm-wTjYLj_BHh8pSG8NwrkwJ99LA6K6M4a9LHQLNSE2BVK5NljgPlz0pYVIxLfS1V9Aq9qB70C9CHY');
            $this->settings->set( 'client_id', 'AXGm-wTjYLj_BHh8pSG8NwrkwJ99LA6K6M4a9LHQLNSE2BVK5NljgPlz0pYVIxLfS1V9Aq9qB70C9CHY');

            $this->settings->set( 'client_secret_sandbox', 'EFO-yQuHgp_IaYCDeS1Jf6gTHAT6TukV3-cmxJeTmhYDdy_55c5lzuZhd3onvC7ZPQs6B2aDSteFjh2f');
            $this->settings->set( 'client_secret_production', 'EFO-yQuHgp_IaYCDeS1Jf6gTHAT6TukV3-cmxJeTmhYDdy_55c5lzuZhd3onvC7ZPQs6B2aDSteFjh2f');
            $this->settings->set( 'client_secret', 'EFO-yQuHgp_IaYCDeS1Jf6gTHAT6TukV3-cmxJeTmhYDdy_55c5lzuZhd3onvC7ZPQs6B2aDSteFjh2f');
        }

        error_log( 'Process (' . $market . ')' );

		if ( ! $order ) {
//            error_log( print_r( $wc_order, true ) );

            $wc_order_refreshed = new WC_Order( $wc_order->get_id() );
			// phpcs:ignore WordPress.Security.NonceVerification
			$order_id = $wc_order_refreshed->get_meta( PayPalGateway::ORDER_ID_META_KEY ) ?: wc_clean( wp_unslash( $_POST['paypal_order_id'] ?? '' ) );
			$order_meta_key_id = $wc_order_refreshed->get_meta( PayPalGateway::ORDER_ID_META_KEY );

//            error_log( 'ORDER META KEY: ' . $order_meta_key_id );
//            error_log( wc_clean( wp_unslash( $_POST['paypal_order_id'] ?? '' ) ) );
//
//			if( !$order_id ) {
