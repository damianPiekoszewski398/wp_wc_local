...
class P24_Multi_Currency implements P24_MC_Interface {
	 ...
	 *
	 * @param string|null $default_currency Default reports currency.
	 *
	 * @return string|null
	 */
	public static function get_admin_reports_currency( $default_currency = null ) {
	 ...
	 *
	 * @param mixed $default_currency Default value provided by filter.
	 * @return string
	 */
	public function try_change_default_currency( $default_currency ) {
		$active = $this->get_active_currency();
		return $active ? $active : $default_currency;
	}

	/**
	 * Try change price format.
	 *
	 * @param mixed  $default_format Default value.
	 * @param string $option Name of option.
	 * @return mixed
	 */
	private function try_change_price_format( $default_format, $option ) {
		...
		return null !== $format ? $format : $default_format;
	}
	 ...
	 *
	 * @param string $separator Default thousand separator.
	 * @return string
	 */
	public function try_change_thousand_separator( $separator ) {
		return $this->try_change_price_format( $separator, 'thousand_separator' );
	}

	/**
	 * Try change decimal separator.
	 *
	 * @param string $separator Default decimal separator.
	 * @return string
	 */
	public function try_change_decimal_separator( $separator ) {
		return $this->try_change_price_format( $separator, 'decimal_separator' );
	}

	/**
	 * Try change number of fraction digits.
	 *
	 * @param mixed $decimals Default number of fraction digits.
	 * @return mixed
	 */
	public function try_change_decimals( $decimals ) {
		return $this->try_change_price_format( $decimals, 'decimals' );
	}

	/**
	 * Try change currency position.
	 *
	 * @param string $position Default currency position.
	 * @return string
	 */
	public function try_change_currency_pos( $position ) {
		return $this->try_change_price_format( $position, 'currency_pos' );
	}
	...
	public function filter_price_hash( $hash, $product, $context ) {
		if ( method_exists( $product, 'get_currency' ) ) {
			...
			if ( ! array_key_exists( $currency, $multipliers ) ) {
				throw new LogicException( esc_html( "The requested currency $currency is not configured." ) );
			}
	...
	public function compute_price_in_currency( $price, $currency ) {
		...
		if ( array_key_exists( $currency, $multipliers ) ) {
			...
			throw new LogicException( esc_html( "The currency $currency not found in config." ) );
		}
	...
	public function convert_price( $price, $from, $to ) {
		...
		if ( array_key_exists( $from, $multipliers ) ) {
			...
			throw new LogicException( esc_html( "The currency $from not found in config." ) );
		}
		if ( array_key_exists( $to, $multipliers ) ) {
			$multiplier_to = $multipliers[ $to ];
		} else {
			throw new LogicException( esc_html( "The currency $to not found in config." ) );
		}
