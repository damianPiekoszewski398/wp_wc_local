...
class Przelewy24Class {
     ...
     * Return host URL
     *
     * @param bool $testMode True for the test URL.
     * @return string
     */
    ...
    public function trnRequest($token, $redirect = true) {
        $url = self::get_trn_request_url_static($token, $this->testMode);
        if($redirect) {
            header('Location: '.$url);
            return '';
        }
    ...
    /**
     * Return URL to a P24 payment screen.
     *
     * @param string $token The token.
     * @param bool $testMode True if in the test mode.
     * @return string
     */
    public static function get_trn_request_url_static($token, $testMode = false) {
        return self::getHostStatic($testMode) .'trnRequest/' . $token;
    }
    ...
    public function availablePaymentMethods($only24at7 = true, $currency = 'PLN', $lang = 'pl')
    {
        ...
        $rest_api = new P24_Rest_Heavy( $this->config );
