...
class P24_Config_Menu {
	...
	private function render_config_mc_main_page() {
		$hpos_active             = P24_Woo_Commerce_Low_Level_Getter::is_hpos_active();
		...
		if ( $multi_currency_instance->is_internal() ) {
			if ( ! $multi_currency_instance instanceof P24_Multi_Currency ) {
				...
				throw new LogicException( esc_html( "The implementation of $class has broken is_internal method or the logic of application has changed." ) );
			}
		...
		$params                           = compact( 'hpos_active', 'multi_currency_instance', 'value', 'order_created_notification_value' );
	...
	private function render_config_main_page_extended( P24_Multi_Currency $multi_currency_instance ) {
		...
		$order_created_notification_value = $this->plugin_core->should_activate_order_created_notification();
		$params                           = compact( 'hpos_active', 'multi_currency_instance', 'value', 'currency_options', 'report_currency', 'order_created_notification_value' );
		$this->plugin_core->render_template( 'multi-currency-main', $params );
	}
