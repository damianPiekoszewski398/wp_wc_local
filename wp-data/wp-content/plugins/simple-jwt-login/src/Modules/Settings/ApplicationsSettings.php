...
class ApplicationsSettings extends BaseSettings implements SettingsInterface
{
    ...
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['enabled'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['enabled']);
    }
    ...
    {
        if (isset($this->settings[self::FACEBOOK_GROUP]['client_id'])) {
            return $this->settings[self::FACEBOOK_GROUP]['client_id'];
        }
    ...
    {
        if (isset($this->settings[self::FACEBOOK_GROUP]['client_secret'])) {
            return $this->settings[self::FACEBOOK_GROUP]['client_secret'];
        }
     ...
     * @return bool
     */
    public function isFacebookJwtAllowedOnAllEndpoints()
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['allow_on_all_endpoints'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['allow_on_all_endpoints']);
    }

    /**
     * @return bool
     */
    public function isFacebookCreateUserIfNotExistsEnabled()
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['create_user_if_not_exists'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['create_user_if_not_exists']);
    }

    public function isFacebookOauthEnabled()
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['enable_oauth'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['enable_oauth']);
    }

    public function isFacebookExchangeCodeEnabled()
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['enable_exchange_code'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['enable_exchange_code']);
    }

    public function getFacebookExchangeCodeRedirectUri()
    {
        if (isset($this->settings[self::FACEBOOK_GROUP]['redirect_uri_exchange_code'])) {
            return $this->settings[self::FACEBOOK_GROUP]['redirect_uri_exchange_code'];
        }

        return  "";
    }

    public function isFacebookExchangeIdTokenEnabled()
    {
        return isset($this->settings[self::FACEBOOK_GROUP]['enable_exchange_id_token'])
            && !empty($this->settings[self::FACEBOOK_GROUP]['enable_exchange_id_token']);
    }
}
