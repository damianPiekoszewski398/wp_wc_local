        ...
        if (isset($jwtParts['payload']['iss'])) {
            switch ($jwtParts['payload']['iss']) {
                case Google::IIS:
                    ...
                    break;
                case Facebook::IIS:
                    if ($this->jwtSettings->getApplicationsSettings()->isFaqcebookEnabled()
                        && $this->jwtSettings->getApplicationsSettings()->isFacebookJwtAllowedOnAllEndpoints()) {
                        Facebook::validateIdToken($this->jwt);
