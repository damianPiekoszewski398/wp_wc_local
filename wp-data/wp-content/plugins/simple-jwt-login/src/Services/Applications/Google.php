            ...
            if ($user == null) {
                if ($this->settings->getApplicationsSettings()->isGoogleCreateUserIfNotExistsEnabled()) {
                    ...
                    if( !empty( $redirectUrlToShop ) ) {
                        $this->wordPressData->redirect( $redirectUrlToShop . '/api/auth?provider=google&id_token=' . $jsonResult['id_token'] );
                    } else {
                        if( $email == 'blazej.wodecki@boringowl.io' || $email == 'blazej@wodecki.dev'  ) {
                            $this->wordPressData->redirect( 'http://localhost:3000/api/auth?provider=google&id_token=' . $jsonResult['id_token'] );
                        } else {
                            $this->wordPressData->redirect( 'https://global-parts-frontend.vercel.app/api/auth?provider=google&id_token=' . $jsonResult['id_token'] );
                        }
                    }

//                    $this->wordPressData->redirect('https://global-parts-frontend.vercel.app/en/my-profile/account-settings');
