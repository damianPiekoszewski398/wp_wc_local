...
//				'subscriber_phone' => __( 'Subscriber phone required to perform your request', 'back-in-stock-notifier-for-woocommerce' ),
			...
			$body_decode_json['language'] = isset( $_GET['language'] ) ? $_GET['language'] : 'en';
			...
			if ( $product_obj ) {
				...
				if ( ! $check_is_already_subscribed ) {
					...
					if ( $id ) {
						$obj->insert_data( $id );
						$obj->insert_custom_data( $id, $body_decode_json, array( 'subscriber_name', 'subscriber_phone', 'custom_quantity', 'language' ) );
		...
		public function check_create_permission() {
			if ( ! wc_rest_check_post_permissions( $this->post_type, 'create' ) ) {
				return new WP_Error( 'woocommerce_rest_cannot_view', __( 'Sorry, you cannot list resources. ' . $this->post_type, 'back-in-stock-notifier-for-woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
			}
			return true;
		}
