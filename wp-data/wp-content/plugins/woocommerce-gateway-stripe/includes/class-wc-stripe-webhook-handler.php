...
class WC_Stripe_Webhook_Handler extends WC_Stripe_Payment_Gateway {
	...
	public function __construct() {
        ...
        /**
         * Nadpisujemy obsługę drugiego konta!!
         */
		if( isset($_GET['market']) && $_GET['market'] == 'eu' ) {
            $stripe_settings['test_publishable_key'] = 'pk_test_51PnGs4Rtk7OZfQQjFKCaWlyCPwpinHTQmllN2boFPFFQHqeNzKqkIo0vfRJvv99g2j04XeESX05yS8JBQjNCVAqC00cfbIIKfB';
            $stripe_settings['test_secret_key'] = 'sk_test_51PnGs4Rtk7OZfQQjs63eLM3ojfiLEU8NhIbGjmOXm47RLDV4ulptLXhQlaGlyDEnXRHtuENPnpYaplP3DygFLFHE00Z3oPbIQK';
            $stripe_settings['test_webhook_secret'] = 'whsec_E015nUyA5rQje9AEmFkjD65ABv7zpF7N';
        }
...
//        error_log( 'WEBHOOK!!!!' );
//        error_log( print_r( $stripe_settings, true ) );
//        error_log( print_r( $_GET, true ) );
	...
	public function process_webhook_charge_succeeded( $notification ) {
		...
		if( ! $order && $notification->data->object->payment_intent ) {
            $order = WC_Stripe_Helper::get_order_by_intent_id( $notification->data->object->payment_intent );

            if( $order ) {
                add_post_meta( $order->get_id(), '_transaction_id', $notification->data->object->id, true );
...
//                $order->delete_meta_data( '_stripe_upe_waiting_for_redirect' );
        ...
        WC_Stripe_Logger::log( 'PRZED REVIEW' );
		...
		if ( 'manual_review' !== $this->get_risk_outcome( $notification ) ) {
            WC_Stripe_Logger::log( 'PAYMENT_COMPLETE! ' );
            ...
            WC_Stripe_Logger::log( 'PAYMENT_INCOMPLETE! ' . $this->get_risk_outcome( $notification ) );
        }
